...
class SpeechEncoder(nn.Module):
    """Modular speech encoder that can use either mHuBERT or Whisper"""
    def __init__(self, config):
        super(SpeechEncoder, self).__init__()
        self.config = config
        
        if config.speech_transformer_chp == config.mHuBERT:
            self.encoder = AutoModel.from_pretrained(config.speech_transformer_chp)
        elif config.speech_transformer_chp == config.WHISPER:
            self.encoder = WhisperModel.from_pretrained(config.speech_transformer_chp).encoder
        else:
            raise ValueError(f"Unknown speech encoder type: {config.speech_transformer_chp}")
        
        self.hidden_size = self.encoder.config.hidden_size
        
    def forward(self, input_values):
        return self.encoder(input_values).last_hidden_state
...
class TBNet(nn.Module):
    def __init__(self, config):
        ...
        speech_embedding_dim = self.speech_encoder.hidden_size
    ...
    def forward(self, input_values, input_ids, demography, attention_mask):
        ...
        speech_embeddings = self.speech_encoder(input_values)
    ...
    def speech_only_forward(self, input_values, return_embeddings=False):
        ...
        transformer_output = self.speech_encoder(input_values)
        output_embeddings = transformer_output
    ...
    def inference(self, audio_path, demography_info, config):
        ...
        waveform = preprocess_audio(audio_path, processor,segment_length=self.config.segment_size, overlap=0.2)
    ...
    def speech_only_inference(self, audio_path, segment_length=5, overlap=0.2, target_sr=16000, device='cuda'):
        ...
        input_values = preprocess_audio(audio_path, processor,segment_length=self.config.segment_size, overlap=0.2)
